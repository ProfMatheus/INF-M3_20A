...
public class Movimenta : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (ControlInput.instanse.iW == true || Input.GetKey(KeyCode.W))
        {
            movimento.transform.Translate((boost+velocidade) * Time.deltaTime, 0, 0);
        }
        else if (ControlInput.instanse.iS == true || Input.GetKey(KeyCode.S))
        {
            movimento.transform.Translate(-((boost + velocidade) * Time.deltaTime), 0, 0);
        }

        if (ControlInput.instanse.iA == true || Input.GetKey(KeyCode.A))
        {
            movimento.transform.Translate(0, 0, (boost + velocidade) * Time.deltaTime);
        }
        else if (ControlInput.instanse.iD == true || Input.GetKey(KeyCode.D))
        {
            movimento.transform.Translate(0, 0, -((boost + velocidade) * Time.deltaTime));
        }

        /*
        if(ControlInput.instanse.iSpace == true || Input.GetKeyDown(KeyCode.Space))
        {
